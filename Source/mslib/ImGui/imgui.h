...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_NoCollapse = 1 << 5,   // ユーザーがマウスホイールで垂直にスクロールできないようにします。子ウィンドウでは、NoScrollbarも設定されていない限り、マウスホイールは親に転送されます。
	ImGuiWindowFlags_AlwaysAutoResize = 1 << 6,   // すべてのウィンドウのサイズをすべてのフレームのコンテンツに合わせて変更します
	...
	ImGuiWindowFlags_NoBringToFrontOnFocus = 1 << 13,  // フォーカスを取得するときにウィンドウを前面に移動することを無効にします（たとえば、クリックするか、プログラムでフォーカスを設定します）
	ImGuiWindowFlags_AlwaysVerticalScrollbar = 1 << 14,  // 常に垂直スクロールバーを表示します（ContentSize.y <Size.yの場合でも）
	...
	// [Internal]
	ImGuiWindowFlags_NavFlattened = 1 << 23,  // [BETA] ゲームパッド/キーボードナビゲーションが親の境界線をこの子に渡ることを許可します（スクロールのない子でのみ使用してください！）
	ImGuiWindowFlags_ChildWindow = 1 << 24,  // 使用しないでください！ BeginChild() による内部使用
	ImGuiWindowFlags_Tooltip = 1 << 25,  // 使用しないでください！ BeginTooltip() による内部使用向け
	ImGuiWindowFlags_Popup = 1 << 26,  // 使用しないでください！ BeginPopup() による内部使用用
	ImGuiWindowFlags_Modal = 1 << 27,  // 使用しないでください！ BeginPopupModal() による内部使用向け
	ImGuiWindowFlags_ChildMenu = 1 << 28,  // 使用しないでください！ BeginMenu() による内部使用用
	ImGuiWindowFlags_DockNodeHost = 1 << 29   // 使用しないでください！ Begin() / NewFrame() による内部使用向け
